...
const createShippingRequestService = async (id: any) => {
  ...
  try {
    ...
    const shippingRequestURL = `${config.shipment_url}/api/shipments/v1/ship`; // Use https
...
const directCreateShippingRequestService = async (id: any) => {
  console.log('Creating shipment request for ID:', id);
  ...
  try {
    ...
    const shipmentCode =
      response.data?.ShipmentResponse?.Response?.ResponseStatus?.Code;
...
export const shippingService = {
  ...
  createShippingRequestService,
  directCreateShippingRequestService,
