...
const getRequiredWorkingtBySellerQuery = async (
  userId: string,
) => {

  let profileUpdate = true;
  let stripeConnectedAccount = true;
  let shopCreateVarify = true;

  const selllerExist: any = await User.findById(userId);
  ...
  for (const field of requiredFields) {
    ...
    if (!selllerExist[field]) {
      // console.log('selllerExist[field]', selllerExist[field]);
      profileUpdate = false;
    }
  }


  const isStripeConnectedAccount:any = await StripeAccount.findOne({
    userId: userId,
  });

  if (!isStripeConnectedAccount) {
    stripeConnectedAccount = false;
  }
  ...
  const isExistShop:any = await Shop.findOne({ sellerId: userId });
  if (!isExistShop) {
    shopCreateVarify = false;
  }
  if (isExistShop?.status !== 'verify') {
    shopCreateVarify = false;
  } 
...
export const productService = {
  ...
  getRequiredWorkingtBySellerQuery,
