...
const getRequiredWorkingtBySeller = catchAsync(async (req, res) => {
  const { userId } = req.user;
  const  result =
    await productService.getRequiredWorkingtBySellerQuery(
      userId,
    );
  ...
  sendResponse(res, {
    ...
    statusCode: httpStatus.OK,
    data: result,
    message: ' All Required work are requered successful!!',
  });
});
